...
// parsed to match the variable definitions, a Error will be returned.
func getVariableValues(schema types.Schema, definitionASTs []*ast.VariableDefinition, inputs map[string]interface{}) (map[string]interface{}, error) {
...
func getArgumentValues(argDefs []*types.Argument, argASTs []*ast.Argument, variableVariables map[string]interface{}) (map[string]interface{}, error) {
...
func getVariableValue(schema types.Schema, definitionAST *ast.VariableDefinition, input interface{}) (interface{}, error) {
...
func coerceValue(ttype types.Input, value interface{}) interface{} {
	if ttype, ok := ttype.(*types.NonNull); ok {
		return coerceValue(ttype.OfType, value)
	}
	if isNullish(value) {
		return nil
	}
	if ttype, ok := ttype.(*types.List); ok {
	...
	if ttype, ok := ttype.(*types.InputObject); ok {
...
func typeFromAST(schema types.Schema, inputTypeAST ast.Type) (types.Type, error) {
	...
	case *ast.ListType:
		...
		return types.NewList(innerType), nil
	case *ast.NonNullType:
		innerType, err := typeFromAST(schema, inputTypeAST.Type)
		if err != nil {
			return nil, err
		}
		return types.NewNonNull(innerType), nil
	case *ast.NamedType:
		nameValue := ""
 ...
 *
 */
func valueFromAST(valueAST ast.Value, ttype types.Input, variables map[string]interface{}) interface{} {
